...
class MF_Actual : AppCompatActivity() {
    ...
    private fun hitungMF() {
        ...
        tvHasilMF.text = "Matching Factor Actual: %.3f".format(hasil)
        ...
        val formattedTelegram = """
            🕒 $timestamp
            [MF Actual]
            Hauler (Baris Pertama): n Aktual=${firstHaulerData.first}, WH Aktual=${firstHaulerData.second}, Pdty Aktual=${firstHaulerData.third}
            Loader (Baris Pertama): n Aktual=${firstLoaderData.first}, WH Aktual=${firstLoaderData.second}, Pdty Aktual=${firstLoaderData.third}
            Total Hauler: ${"%.3f".format(totalHauler)}
            Total Loader: ${"%.3f".format(totalLoader)}
            Matching Factor Actual: ${"%.3f".format(hasil)}
            ------------------------------------
        """.trimIndent()
